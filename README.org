...
* Introduction
  ...
  {{{org}}}, I store the documentation and configuration code in
  =README.org= file which is loaded =init.el=.  This project is hosted
  on https://github.com/gongzhitaao/dotemacs.
...
* Bootstrap
  ...
  The only thing done in =init.el= is to load this file, =README.org=,
  which includes all our documented configurations.

  #+INCLUDE: "init.el" src emacs-lisp :lines "6-"

  Notice that I load two packages, =cask= and =pallet=, which are
