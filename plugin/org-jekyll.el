...
See `org-publish-org-to' to the list of arguments."
  (org-publish-with-aux-preprocess-maybe
   (org-publish-org-to "jekyll" plist filename pub-dir)))

(defun org-export-as-jekyll (arg &optional hidden ext-plist
...
PUB-DIR is set, use this as the publishing directory."
  ...
  (setq origin-file-name (file-name-nondirectory buffer-file-name))
  (setq export-file-name
        (format "%s%s"
                       ...
                       (file-name-sans-extension
                        (or (and subtree-p
                                 (org-entry-get (region-beginning)
                                                "EXPORT_FILE_NAME" t))
                            (file-name-nondirectory export-file-name)))
                       "." html-extension)
...
---
"
                 (or (plist-get opt-plist :jekyll-layout)
                     "default")
                 (or (read-from-minibuffer
                      (format "Title of %s: " origin-file-name))
                     origin-file-name)
                 (or (plist-get opt-plist :jekyll-category)
                     (read-from-minibuffer
                      (format "Category of %s: " origin-file-name)))
                 (or (plist-get opt-plist :jekyll-tags)
                     (read-from-minibuffer
                      (format "Tags of %s: " origin-file-name)))
                 date
                 ))
